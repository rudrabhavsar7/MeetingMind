...
class SMTPPasswordResetNotifier:
    ...
    def _send_sync(self, dispatch: PasswordResetDispatch) -> None:
        reset_url = f"{self._settings.frontend_url.rstrip('/')}/reset-password#{urlencode({'token': dispatch.token})}"
...
class SMTPWorkspaceInvitationNotifier:
    ...
    def _send_sync(self, dispatch: WorkspaceInvitationDispatch) -> None:
        register_url = f"{self._settings.frontend_url.rstrip('/')}/register#{urlencode({'token': dispatch.token})}"
        message = EmailMessage()
