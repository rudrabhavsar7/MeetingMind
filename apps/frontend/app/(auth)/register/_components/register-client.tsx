...
export default function RegisterClient() {
  ...
  useEffect(() => {
    if (token) {
      router.push(`/register/invitation?token=${encodeURIComponent(token)}`);
    }
  }, [token, router]);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Re-check bootstrap status to handle setup-race condition
    try {
      const currentStatus = await getBootstrapStatus();
      if (!currentStatus.setup_required) {
        setSubmitError("Workspace setup has been completed. Please use an invitation link to register.");
        return;
      }
    } catch {
