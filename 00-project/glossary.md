...
**Empty State**
The UI displayed when a view has no data (e.g., no meetings uploaded, or no search results). Designed to guide the user toward the next logical action.

**Knowledge Base**
The collective, searchable repository of all processed meetings within a Workspace.

**Meeting**
The core data entity in MeetingMind, representing an uploaded audio/video file and all its derived intelligence (transcript, summary, actions, decisions).
...
**MinIO**
The S3-compatible object storage server used in self-hosted deployments to store raw audio/video uploads and processed media files.
...
**Presigned URL**
A temporary, secure URL generated by MinIO/S3 that allows the frontend to upload files directly to storage without passing through the backend API.
